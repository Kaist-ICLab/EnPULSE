...
class BenchmarkService : Service() {
    ...
    companion object {
        ...
        const val ACTION_PAUSE = "kaist.iclab.benchmark.PAUSE"
        const val ACTION_RESUME = "kaist.iclab.benchmark.RESUME"
    ...
    override fun onDestroy() {
        ...
        if (startTimeMs > 0 && initialBatteryLevel != -1) {
            ...
            val droppedCharge = if (initialBatteryChargeUah != -1L && latestBatteryChargeUah != -1L) {
            ...
            val summaryText = buildString {
                ...
                appendLine("Paused Dur:      ${String.format("%.2f", totalPausedMs / (1000.0 * 3600.0))} hours")
                ...
                appendLine("Max CPU Temp:    ${if (maxCpuTemperature >= 0) String.format("%.1f", maxCpuTemperature) + " C" else "N/A"}")
                appendLine("Max App Memory:  ${String.format("%.1f", maxAppMemoryMb)} MB")
                appendLine("Max Native Heap: ${String.format("%.1f", maxNativeHeapBytes / (1024f * 1024f))} MB")
    ...
    private fun buildNotification(text: String, batteryLevel: Int): Notification {
        ...
        val pauseActionStr = if (isPaused) "Resume" else "Pause"
        val pauseIntent = Intent(this, BenchmarkService::class.java).apply {
        ...
        return Notification.Builder(this, CHANNEL_ID)
            ...
            .addAction(
                Notification.Action.Builder(
                    null, pauseActionStr, pausePending
                ).build()
            )
            .addAction(
                Notification.Action.Builder(
