...
| --- |--------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| --- |
| `ActivityRecognitionSensor` | Phone  | Coarse physical-activity classification via Google ActivityRecognitionAPI (still / walking / running / cycling / vehicle / tilting).                                          | `activityType`, `score`, `probabilities` |
...
| `MicroEmaSensor` | Galaxy Watch | Watch-side micro-EMA prompts triggered by rules and delivered over BLE; responses are surfaced as `SurveySensor.Entity`.                                                      | `response`, `triggerTime`, `responseTime` |
