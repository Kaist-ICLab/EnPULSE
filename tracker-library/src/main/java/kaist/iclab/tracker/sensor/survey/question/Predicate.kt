...
sealed class Operator<T> : Expression<T>() {
    data class And<T : Comparable<T>>(val a: ValueComparator<T>, val b: ValueComparator<T>) :
        Operator<T>()

    data class Or<T : Comparable<T>>(val a: ValueComparator<T>, val b: ValueComparator<T>) :
        Operator<T>()
