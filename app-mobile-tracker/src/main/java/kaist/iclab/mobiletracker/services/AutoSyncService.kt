 ...
 * interval and network preferences.
 *
 * A real, persistent foreground service (per main's `c054a111` fix) rather than a plain
 * background `Service`, so it isn't as readily killed by Doze/App Standby while the app has no
 * visible UI — this is what keeps the auto-sync loop itself alive. The actual upload work — shared
 * with the manual "Upload Now" action — is delegated to [DataUploadService], a separate foreground
 * service with its own progress/result notification, so this service's own notification only ever
 * says "auto-sync is active", not per-cycle success/failure.
 *
...
class AutoSyncService : LifecycleService(), KoinComponent {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        startForeground(Constants.Notification.ID_AUTO_SYNC_ACTIVE, notification, serviceType)
