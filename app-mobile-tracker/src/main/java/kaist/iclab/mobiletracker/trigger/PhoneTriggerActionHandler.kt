 ...
 * here except [TriggerActionConfig.WatchEma], which needs one BLE hop to tell the watch to launch
 * its MicroEMA UI (mirrors [kaist.iclab.wearabletracker] `WatchEmaTriggerReceiver` on the other end).
 */
class PhoneTriggerActionHandler(
    ...
    override suspend fun onAction(trigger: ParsedCampaignTrigger, action: TriggerActionConfig) {
        when (action) {
            ...
            is TriggerActionConfig.Notification -> handleNotification(action)
        }
    }
    ...
    private fun handleNotification(action: TriggerActionConfig.Notification) {
